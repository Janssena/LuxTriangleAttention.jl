                ...
                for (name, mask) in mask_cfg
                    @testset "$name" begin
                        for T in [Float16, Float32, Float64]
                            ...
                            jl_layer = Attention(chn_in, head_dim, num_heads; use_bias=(false, (gate=true, out=true)), bias_layout)
                            ps, st = Lux.setup(rng, jl_layer) |> convert_types(T)

                            py_layer = py"AF2Attention"(chn_in, chn_in, chn_in, head_dim, num_heads)
